...
re_image_wl = re.compile(r"^(?:File|Image|Media):", flags=re.IGNORECASE | re.UNICODE)
...
class Wikipedia(Dataset):
    ...
    def __init__(
        ...
        super(Wikipedia, self).__init__(NAME, meta=META)
        try:
        ...
        self._filestub = "{lang}{project}/{version}/{lang}{project}-{version}-pages-articles.xml.bz2".format(
    ...
    def _check_file_url(self, file_url):
        ...
        if response.status_code != 200:
            ...
            response = requests.head(lang_project_url)
            if response.status_code != 200:
                raise ValueError(
                ...
                raise requests.HTTPError(
                    "'{lang}' variant of '{project}' does exist, "
                    "but no file found at {url}".format(url=file_url)
                )
    ...
    def __iter__(self):
        ...
        with f:
            ...
            title_path = "./{{{}}}title".format(namespace)
            text_path = "./{{{}}}revision/{{{}}}text".format(namespace, namespace)
            for elem in elems:
                if elem.tag == page_tag:
                    ns = elem.findtext(ns_path, default="")
                    if ns != self.namespace:
                        elem.clear()  # remove this part of the tree
                        continue
                    page_id = elem.findtext(page_id_path, default=None)
                    title = elem.findtext(title_path, default=None)
                    text = elem.findtext(text_path, default="")
                    text = compat.to_unicode(text, errors="ignore")  # not ideal
                    elem.clear()  # remove this part of the tree
    ...
    def _get_parse_func(self, fast, include_headings):
        if fast is True:
            def fast_parse(include_headings, record):
                if include_headings is True:
                    record["text"] = (
                        record["title"] + "\n\n"
                        + strip_markup(record["text"], include_headings)
                    )
                else:
                    record["text"] = strip_markup(record["text"], include_headings)
                return record

            return functools.partial(fast_parse, include_headings)
            ...
            try:
                ...
                raise ImportError(
                    "`mwparserfromhell` is not installed, so `fast=False` won't work; "
            ...
            return functools.partial(slow_parse, include_headings, parser)

    def _parsed_filtered_iter(self, parse_func, filters):
        if filters:
            for record in self:
                parsed_record = parse_func(record)
                if all(filter_(parsed_record) for filter_ in filters):
                    yield parsed_record
        else:
            for record in self:
                yield parse_func(record)

    def texts(self, fast=True, include_headings=True, min_len=None, limit=None):
        """
        ...
        in order of appearance in the database dump file.
        ...
        Args:
            ...
            include_headings (bool): If True, include page and section headings
                as part of the page's text; otherwise, filter them out.
            min_len (int): Filter wiki pages for those whose body length in chars
        ...
        """
        filters = self._get_filters(min_len)
        parse_func = self._get_parse_func(fast, include_headings)
        for record in itertools.islice(self._parsed_filtered_iter(parse_func, filters), limit):
            yield record["text"]

    def records(self, fast=True, include_headings=True, min_len=None, limit=None):
        """
        Iterate over wiki pages in this dataset, optionally filtering by a variety
...
def strip_markup_slow(wikicode, include_headings, parser):
    """
    Args:
        wikicode (str or :class:`mwparserfromhell.wikicode.Wikicode`)
        include_headings (bool)
        parser (:class:`mwparserfromhell.parser.Parser`)

    Returns:
        str
    """
    if isinstance(wikicode, compat.string_types):
        if parser is None:
            raise ValueError()
        else:
            wikicode = parser.parse(wikicode)
    ...
    def is_bad_wikilink(obj):
        return bool(re_image_wl.match(compat.unicode_(obj.title)))

    def is_bad_tag(obj):
        return compat.unicode_(obj.tag) in bad_tags
...
def strip_markup(wikitext, include_headings):
    """
