...
import textacy
from textacy import compat, datasets, keyterms
...
def test_default(spacy_doc):
    result = keyterms.sgrank(spacy_doc)
...
def test_ngrams_1(spacy_doc):
    result = keyterms.sgrank(spacy_doc, ngrams=1)
    assert len(result) > 0
    assert all(len(term.split()) == 1 for term, _ in result)


def test_ngrams_2_3(spacy_doc):
    result = keyterms.sgrank(spacy_doc, ngrams=(2, 3))
    assert len(result) > 0
...
def test_n_keyterms(spacy_doc):
    for n in (5, 25):
        result = keyterms.sgrank(spacy_doc, n_keyterms=n)
        assert 0 < len(result) <= n


def test_n_keyterms_float(spacy_doc):
    result = keyterms.sgrank(spacy_doc, n_keyterms=0.2)
    assert len(result) > 0
    with pytest.raises(ValueError):
        _ = keyterms.sgrank(spacy_doc, n_keyterms=2.0)


def test_normalize_lower(spacy_doc):
    result = keyterms.sgrank(spacy_doc, normalize="lower")
    assert len(result) > 0
    assert all(term == term.lower() for term, _ in result)


def test_normalize_none(spacy_doc):
    result = keyterms.sgrank(spacy_doc, normalize=None)
    assert len(result) > 0
    assert any(term != term.lower() for term, _ in result)


def test_normalize_callable(spacy_doc):
    result = keyterms.sgrank(spacy_doc, normalize=lambda tok: tok.upper_)
    assert len(result) > 0
    assert all(term == term.upper() for term, _ in result)


def test_window_width(spacy_doc):
    result_10 = keyterms.sgrank(spacy_doc, window_width=10)
    result_100 = keyterms.sgrank(spacy_doc, window_width=100)
    assert len(result_10) > 0 and len(result_100) > 0
    assert result_10 != result_100
    with pytest.raises(ValueError):
        _ = keyterms.sgrank(spacy_doc, window_width=1)


def test_empty_doc(empty_spacy_doc):
    result = keyterms.sgrank(empty_spacy_doc)
