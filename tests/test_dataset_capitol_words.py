...
def test_records_speaker_name():
    ...
    for speaker_name in speaker_names:
        assert all(r['speaker_name'] in speaker_name
...
def test_records_speaker_party():
    ...
    for speaker_party in speaker_parties:
        assert all(r['speaker_party'] in speaker_party
...
def test_records_chamber():
    ...
    for chamber in chambers:
        assert all(r['chamber'] in chamber
...
def test_records_congress():
    ...
    for congress in congresses:
        assert all(r['congress'] in congress
...
def test_records_date_range():
    ...
    for date_range in date_ranges:
        assert all(date_range[0] <= r['date'] < date_range[1]
