...
def test_words(spacy_doc):
    ...
    observed = [
        ...
        for tok in extract.words(
            spacy_doc, filter_stops=False, filter_punct=False, filter_nums=False
        )
    ][:25]
...
def test_ngrams_n(spacy_doc):
    for n in (1, 2):
        result = [
            span
            for span in extract.ngrams(
                spacy_doc, n, filter_stops=False, filter_punct=False, filter_nums=False
            )
        ]
...
def test_named_entities(spacy_doc):
    result = [ent for ent in extract.named_entities(spacy_doc, drop_determiners=False)]
...
def test_named_entities_min_freq(spacy_doc):
    ...
    observed = [
        ent.text
        for ent in extract.named_entities(spacy_doc, drop_determiners=True, min_freq=2)
    ]
    assert observed == expected
...
def test_noun_chunks_determiner(spacy_doc):
    ...
    observed = [
        nc.text for nc in extract.noun_chunks(spacy_doc, drop_determiners=False)
    ][:10]
    assert observed == expected
...
def test_noun_chunks_min_freq(spacy_doc):
    ...
    observed = [
        nc.text
        for nc in extract.noun_chunks(spacy_doc, drop_determiners=True, min_freq=2)
    ]
    assert observed == expected
...
def test_subject_verb_object_triples(spacy_doc):
    expected = [
        "we, discussed, impact",
        "education official, raised, hand",
        "he, could ask, me",
        "he, could ask, question",
        "we, need, to close",
    ]
    observed = [
