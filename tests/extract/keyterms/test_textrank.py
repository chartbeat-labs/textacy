...
def test_n_topn(spacy_doc):
    for n in (5, 25):
        result = kt.textrank(spacy_doc, topn=n)
        assert 0 < len(result) <= n


def test_topn_float(spacy_doc):
    result = kt.textrank(spacy_doc, topn=0.2)
    assert len(result) > 0
    with pytest.raises(ValueError):
        _ = kt.textrank(spacy_doc, topn=2.0)


def test_window_size(spacy_doc):
    result1 = kt.textrank(spacy_doc, window_size=2)
    result2 = kt.textrank(spacy_doc, window_size=4)
    assert len(result1) > 0 and len(result2) > 0
    assert result1 != result2


def test_edge_weighting(spacy_doc):
    result1 = kt.textrank(spacy_doc, edge_weighting="binary")
    result2 = kt.textrank(spacy_doc, edge_weighting="count")
    assert len(result1) > 0 and len(result2) > 0
    assert result1 != result2


def test_position_bias(spacy_doc):
    result1 = kt.textrank(spacy_doc, position_bias=False)
    result2 = kt.textrank(spacy_doc, position_bias=True)
    assert len(result1) > 0 and len(result2) > 0
    assert result1 != result2
